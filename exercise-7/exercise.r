...
team.data <- mutate(team.data, TOVSTL = TOV/STL)

# Sort the teams from lowest turnover/steal ratio to highest
team.data <- arrange(team.data, TOV-STL)

# Which team has the lowest ratio?
as.character(team.data[1, 2])
...
team.data <- mutate(team.data, AsstG = AST / G) %>%
  arrange(-AsstG)
...
BetterShooters('Golden State Warriors*', 'CLeveland Cavaliers*')
